...
class PositionManager:
    ...
    def get_metrics(self, hours: float = 24.0) -> Dict[str, Any]:
        """直近指定時間（デフォルト24h）のPF、MaxDD、勝率、取引数を計算"""
        with self.lock:
            cutoff = time.time() - hours * 3600.0
            recent = [t for t in self.trades_history if t.get("timestamp", 0) >= cutoff]
            cnt = len(recent)
            if cnt == 0:
                return {
                    "trades_count": 0,
            ...
            win_cnt = len(wins)
            win_rate = (win_cnt / cnt * 100.0) if cnt > 0 else 0.0
            ...
            # Max Drawdown (直近24hの累積損益カーブにおけるピークからの最大下落幅)
            cum = 0.0
            peak = 0.0
            max_dd = 0.0
            for t in recent:
    ...
    def get_snapshot(self, mid_price: float = 0.0) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            metrics = self.get_metrics(hours=24.0)
            return {
